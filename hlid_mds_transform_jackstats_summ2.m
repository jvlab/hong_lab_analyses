...
% and specailized to plot_mode=2
%
% jack_quantiles (error bars) defaults to [0.25 0.75]
%
...
if ~exist('jack_quantiles') jack_quantiles=[0.25 0.75]; end %
njq=length(jack_quantiles);
%
...
results.magnif_summ_dims_inside='d1: ref dim= adj dim, [d2: eiv number] [jack]';
...
results.projs_summ_dims_inside='d1: ref dim= adj dim, d2: 1->ref, 2->adj; then stim and coord and [jack]';
            ...
            for iu2=1:nu2
                ...
                for idim_ptr=1:length(dimlist)
                    ...
                    for ijack=1:results.nstims
                        ...
                        %flip the projections if needed to align with original data
                        for ira=1:2
                            pjack(keep,:,ira)=jack_base.(ra_text{ira}).projections{imodel};
                            pjack(ijack,:,ira)=NaN;
                            %these two lines flip the entire projection
                                ...
                                if ~isempty(istim)
                                    ...
                                    for ijq=1:njq
                                        plot(repmat(xplot_off+istim_name,1,2)+hoff_withinproj*[-0.5 0.5],...
                                            yplot_off+repmat(quantile(squeeze(p.projs_jack{idim,ira}(istim,dproj,:)),jack_quantiles(ijq))/proj_max,1,2),...
                            ...
                            for dproj=1:idim
                                ...
                                switch isp
                                    case {1,2} %norm dot products
                                        ...
                                        proj_jack=reshape(p.projs_jack{idim,isp}(:,dproj,:),size(p.projs_jack{idim,isp},1),results.nstims);
                                        data_exp=1;
                                        data_jack=zeros(1,results.nstims);
                                        for ijack=1:results.nstims %remove the missing stim before the dot-product
                                            keep=setdiff([1:results.nstims],ijack); 
                                            proj_exp_keep=proj_exp(keep); %full data projection as a column, jackknifed stim removed
                                            proj_jack_keep=proj_jack(keep,ijack); %a column with the nan removed
                                            data_jack(ijack)=sum(proj_exp_keep.*proj_jack_keep)./sqrt(sum(proj_exp_keep.^2))./sqrt(sum(proj_jack_keep.^2));
                                        end
                                    case 3 %magnif factor
                                        data_exp=m.magnif_all(idim,dproj); %01Jan26: first index was wrongly dproj
                                        data_jack=squeeze(m.magnif_all_jack(idim,dproj,:)); %01Jan26: first index was wrongly dproj
                                end
                                %plot histogram "by hand", so we can control offset
                                hist_data_raw=histc(data_jack,bin_edges)';
                                ...
                                plot(quantile(data_jack,jack_quantiles([1 end])),repmat(yplot_off+voff_withinproj*dproj,1,2),'Color',color); %show the inter-quantile range
                                for ijq=1:njq
                                    plot(repmat(quantile(data_jack,jack_quantiles(ijq)),1,2),yplot_off+voff_withinproj*(dproj+[-0.5 0.5]),'Color',color);
                                end
                            end %dproj (projection dimension)
                        end %idim_ptr
