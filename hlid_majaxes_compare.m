%hlid_majaxes_compare: compare major axes of a transformation and axes determined from 
% connectivity matrix
% 
% uses a file saved by hlid_majaxes, containing axes of a transformation
% for plots in stimulus space, kc order is used
%
% 20Oct24: add bootstrap for range of correlation values
%
...
if ~exist('nboots') nboots=1000; end %number of boots to compute
    ...
    if ~isempty(d_adj_ptr) & ~isempty(d_ref_ptr)
        ...
        if if_roimatch
           conn_eivecs=conn_use*c_data.eigenvectors(:,1:neivs_connect); %eigenvectors of connectivity matrix, mapped to roi
            ...
            for k=1:nshuffs %shuffle the roi label
                corrs_shuff(:,:,k)=abs(corr(projections_roi(randperm(nrois),:),conn_eivecs));
            end
            corrs_boot=zeros(d_adj,neivs_connect,nboots);           
            for k=1:nboots %randomly draw from responses and eivecs from corresponding glomeruli
                boot_draw=ceil(nrois*rand(nrois,1));
                corrs_boot(:,:,k)=abs(corr(projections_roi(boot_draw,:),conn_eivecs(boot_draw,:)));
            end
        end
        %
        figure;
        ...
        if if_roimatch %further calculations if all rois match to connection tags
            ...
            for k=1:d_adj
                ...
                legt=cell(1,1+(double(nshuffs>1)+double(nboots>1))*length(quantiles_plot));
                plot([1:neivs_connect],corrs_data(k,:),'k');
                legt{1}='data';
                hold on;
                leg_ptr=1;
                if (nshuffs>0)
                    for iq=1:length(quantiles_plot) %plot upper quantile for shuffle
                        plot([1:neivs_connect],quantile(corrs_shuff(k,:,:),1-quantiles_plot(iq),3),'m--');
                        leg_ptr=leg_ptr+1;
                        legt{leg_ptr}=sprintf('shuf q %5.3f',quantiles_plot(iq));
                    end
                end
                if (nboots>0)
                    for iq=1:length(quantiles_plot) %plot lower quantile for bootstrap
                        plot([1:neivs_connect],quantile(corrs_boot(k,:,:),quantiles_plot(iq),3),'b--');
                        leg_ptr=leg_ptr+1;
                        legt{leg_ptr}=sprintf('boot q %5.3f',quantiles_plot(iq));
                    end
                end
        ...
        text(0,0,sprintf('nshuffs: %7.0f, nboots: %.0f',nshuffs,nboots));
        axis off;
        %
        colormap jet;
    else
