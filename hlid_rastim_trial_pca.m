...
%
% to do:PCA, possibly summaries of regression coefs
%
...
for ifile=1:nfiles
    for idist=1:ndists
        ...
        for isub=1:nsubs
            ...
            %compute resp_ortho: subtract off best-fitting multiple of mean response
            ...
            resp_ortho=resp_trial_reshape-repmat(resp_trial_regress,[1 size(resp_stim,2)]).*resp_stim(ind_replic,:); %component of trial response orthogonal to mean
            switch dist_labels{idist}
                case 'Euclidean'
                    ...
                    dists{ifile,idist,isub,5}=sqrt(min(cootodsq(resp_ortho,resp_ortho),cootodsq(resp_ortho,-resp_ortho)));
                    dists{ifile,idist,isub,6}=sqrt(min(cootodsq(resp_stim,resp_ortho),cootodsq(resp_stim,-resp_ortho)));
                case '1-correl'
                    ...
                    dists{ifile,idist,isub,5}=1-abs(corr(resp_ortho',resp_ortho'));
                    dists{ifile,idist,isub,6}=1-abs(corr(resp_stim',resp_ortho'));
            end
        end %row (no mean sub or men sub)
    end %idist
end %ifile
%
%plot
%
[nrows,ncols]=nicesubp(nprocs,0.7);
for isub=1:nsubs
    for idist=1:ndists
        for ifile=1:nfiles+1
            if ifile<=nfiles
                ...
                tstring=sprintf('average across files, dist: %s %s',dist_labels{idist},sub_labels{isub});
            end
            figure;
