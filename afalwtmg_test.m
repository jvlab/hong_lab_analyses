% afalwtmg_test: test weighted affine alignments afalwtm and afalwtmg
% (multi-factor PCA with missing data, greedy variant)
% 
%  afalwtmg allows multiple components, and an affine term.
%    differs from afalwtm in that it also uses a "greedy" algorithm
%    when more than one component is requested
%
...
%
%    See also:   AFALTWMG, AFALWTM_TEST, AFALWT_TEST, AFALWTM_INIT, AFALWTM_ITER,
%         AFALWTM, AFALWTM_WCA, GRMSCMDT.
%
...
%
pcastd=afalwtm_init(d,npcs_model,exact.b_norm);
%
...
if (wthr>0)
    w=double(w<wthr);
    % ensure that there are at least npcs_model ratings in each row
    for f=[find(sum(w,2)<npcs_model)]';
        w(f,:)=double(randperm(nr)<=npcs_model);
    end
end
%
...
%
disp(' running afalwtmg')
[pcawt_g,b_change_g,optsused_g]=afalwtmg(dm,w,npcs_fit,afalwt_opts);
disp(optsused_g);
disp(' running afalwtmg with noconst=1')
[pcawt_g_noconst,b_change_g_noconst,optsused_g_noconst]=afalwtmg(dm,w,npcs_fit,setfield(afalwt_opts,'noconst',1));
disp(optsused_g_noconst);
